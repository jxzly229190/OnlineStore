        ...
        switch (type) {
            case "telphone":
                if (!/^1[3|4|5|8][0-9]\d{4,8}$/.test(val)) {
                    validate.info($this, "手机号码有误，请重新输入!");
                    success = false;
                }
                break;
            case "phone":
                if (!/^(([0\+]\d{2,3}-?)?(0\d{2,3})-?)(\d{7,8})(-?(\d{3,}))?$/.test(val)) {
                    validate.info($this, "电话号码有误，请重新输入!");
                    success = false;
                }
                break;
            case "email":
                if (!/^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/.test(val)) {
                    validate.info($this, "邮件地址有误，请重新输入!");
                    success = false;
                }
                break;
            case "qq": //QQ号码限制为5到12位
                if (!/^[1-9][0-9]{4,11}$/.test(val)) {
                    validate.info($this, "QQ号码有误，请重新输入!");
                    success = false;
                }
                break;
            case "chineseName":
                if (!/^[\u4E00-\u9FA5\uf900-\ufa2d]{2,6}$/.test(val)) {
                    validate.info($this, "名称输入有误，请重新输入!");
                    success = false;
                }
                break;
            case "integer": //正整数限制为0-2147483648
                if (!/^[1-9]\d*$/.test(val)) {
                    validate.info($this, "正整数输入有误，请重新输入!");
                    success = false;
            ...
            case "float":
                if (!/^[1-9]\d*$|^[1-9]\d*\.\d{1,4}$/.test(val)) {
                    validate.info($this, "浮点数输入有误，请重新输入!");
                    success = false;
