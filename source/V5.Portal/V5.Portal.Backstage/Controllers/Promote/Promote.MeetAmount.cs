...
namespace V5.Portal.Backstage.Controllers.Promote
{
    ...
    public partial class PromoteController
    {
        ...
        /// </returns>
        [HttpPost]
        public JsonResult MeetAmountDetail(int id)
        {
            PromoteMeetAmountModel promoteMeetAmountModel;
            try
            {
                this.promoteMeetAmountService = new PromoteMeetAmountService();
                var promoteMeetAmount = this.promoteMeetAmountService.QueryByID(id);
                ...
                if (promoteMeetAmount != null)
                {
                    ...
                    var meetAmountScope = this.promoteMeetAmountScopeService.QueryByMeetAmountID(id);
            ...
            return this.Json(promoteMeetAmountModel);
        }
            ...
            List<PromoteMeetAmountRuleModel> contents)
        {
            try
            {
                ...
                var promoteMeetAmount = DataTransfer.Transfer<Promote_MeetAmount>(
                    promoteMeetAmountModel,
                    typeof(PromoteMeetAmountModel));
                meetAmountScopeModel.MeetAmountID = promoteMeetAmountModel.ID;
                promoteMeetAmount.MeetAmountScope = DataTransfer.Transfer<Promote_MeetAmount_Scope>(
                    meetAmountScopeModel,
                    typeof(PromoteMeetAmountScopeModel));
        ...
        /// The <see cref="string"/>.
        /// </returns>
        private string GetAmountOperationHtml(int id, int status, DateTime startTime, DateTime endTime)
        {
            ...
            if (endTime > DateTime.Now)
            {
                switch (status)
                {
                    case 1:
                        if (startTime > DateTime.Now)
                        {
                            sb.Append(
                                ...
                                + "' value='编辑' onclick='meetAmount.MeetAmountEdit(this)'style='"
                                + permissionUtility.GetDisplayAttribute("ModifyMeetAmount", "Promote", "POST")["style"]
                                + "'/>");
                        }

                        sb.Append(
                            "<input type='button' class='k-button k-grid-suspend' name='" + id
                            + "'  value='暂停' onclick='meetAmount.MeetAmountSuspend(this)'style='"
                            + permissionUtility.GetDisplayAttribute("suspendmeetamount", "Promote", "POST")["style"]
                            + "'/>");
                        sb.Append(
                            "<input type='button' class='k-button k-grid-stop' name='" + id
                            + "'  value='停止' onclick='meetAmount.MeetAmountStop(this)'style='"
                            + permissionUtility.GetDisplayAttribute("stopmeetamount", "Promote", "POST")["style"]
                            + "'/>");
                        break;
                    case 2:
                        if (startTime > DateTime.Now)
                        ...
                        sb.Append(
                            "<input type='button' class='k-button k-grid-suspend' name='" + id
                            + "'  value='恢复' onclick='meetAmount.MeetAmountSuspend(this)'style='"
                            + permissionUtility.GetDisplayAttribute("recovermeetamount", "Promote", "POST")["style"]
                            + "'/>");
                        sb.Append(
                            "<input type='button' class='k-button k-grid-stop' name='" + id
                            + "'  value='停止' onclick='meetAmount.MeetAmountStop(this)'style='"
                    ...
                    case 3:
                        sb.Append(
                            "<input type='button' class='k-button k-grid-stop' name='" + id
                            + "'  value='重做' onclick='meetAmount.MeetAmountEdit(this,1)' style='"
                            + permissionUtility.GetDisplayAttribute("addmeetamount", "Promote", "POST")["style"] + "'/>");
                        break;
                }
            }
            else
            ...
            sb.Append(
                ...
                + "' value='查看' onclick='meetAmount.MeetAmountDetail(this)'style='"
                + permissionUtility.GetDisplayAttribute("meetamountdetail", "Promote", "GET")["style"] + "'/>");
            sb.Append(
                "<input type='button' class='k-button k-grid-delete' name='" + id
                + "' value='删除' onclick='meetAmount.MeetAmountDelete(this)'style='"
                + permissionUtility.GetDisplayAttribute("removemeetamount", "Promote", "POST")["style"] + "'/>");
